...
class Actions(Enum):
    ...
    down  = 2 # move down
    left  = 3 # move left
    none  = 4 # none or pick or drop

class OfficeWorldEnv:
    ...
    def get_mdp_label(self, s, s_next, u):
        ...
        if (row, col) in self.env_settings['decorations']:
            return ['d']

        thresh = 0.3 #0.3

        if self.agent_id == 1:
            if u == 0 and np.random.random() <= thresh:
                l.append('bob')
                self.unlock_wall()
            if u == 1 and (row, col) == self.env_settings['coffee'] \
                        and (prev_row, prev_col) == self.env_settings['coffee']:
                l.append('c')
            if u == 2 and (row, col) == self.env_settings['office'] \
                        and (prev_row, prev_col) == self.env_settings['office']:
                l.append('o')
        elif self.agent_id == 2:
            if u == 0 and (row, col) == self.env_settings['blue_button'] \
                        and (prev_row, prev_col) == self.env_settings['blue_button']:
                l.append('bb')
            if u == 1 and (row, col) == self.env_settings['orange_button'] \
                        and (prev_row, prev_col) == self.env_settings['orange_button']:
                l.append('bob')
    ...
    def get_next_state(self, s, a):
        ...
        if self.agent_id == 2:
            if self.u == 0 or self.u == 1:
                if (row, col) in self.blueorange_tiles:
                    s_next = s
        if not (self.blue_button_pushed and self.orange_button_pushed):
            if (row, col) in self.blueorange_tiles:
                s_next = s
    ...
    def unlock_wall(self):
        """
        Unlocks the blue-orange wall during training.
        Returns
        -------
    ...
    def get_strategy_training_mdp_label(self, s, s_next, option):
        """
        Return the label of the next environment state and current RM state.
        """
        row, col = self.get_state_description(s_next)
        ...
        if option == 'c':
            if np.random.random() <= thresh:
                l.append('bob')
            if (row, col) == self.env_settings['coffee'] \
                        and (prev_row, prev_col) == self.env_settings['coffee']:
                l.append('c')
        elif option == 'o':
            if (row, col) == self.env_settings['office'] \
                        and (prev_row, prev_col) == self.env_settings['office']:
                l.append('o')
        elif option == 'bb':
            ...
            if (row, col) == self.env_settings['orange_button'] \
                        and (prev_row, prev_col) == self.env_settings['orange_button']:
                l.append('bob')

        return l

    def get_strategy_training_next_state(self, s, a):
        """
        Get the next state in the environment given action a is taken from state s.
        Update the last action that was truly taken due to MDP slip.
        ...
        # If the two buttons haven't been pressed in order yet, don't allow the agent into the colored region
        if not (self.blue_button_pushed and self.orange_button_pushed) and (row, col) in self.blueorange_tiles:
            s_next = s
    ...
    def hrm_environment_step(self, s, a, option=None):
        ...
        s_next : int
            Index of next state.
        """
        s_next, last_action = self.get_strategy_training_next_state(s, a)
        self.last_action = last_action
...
def play():
    ...
    rm_string = os.path.join(base_file_dir, 'experiments', 'officeworld', 'officeworld_rm_agent_{}.txt'.format(agent_id))
    ...
    game = OfficeWorldEnv(rm_string, agent_id, env_settings)
